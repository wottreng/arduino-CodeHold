...
int historyIndex = 0;
bool historyIndexMaxed = false;
const int tempHistoryLength = 200;
float tempHistory[tempHistoryLength] = {}; // 200 vars of 4 bytes
char tempHistoryTime[tempHistoryLength][6]; // 200 vars of 6 bytes
unsigned long historyTrackingTimeInterval = 180000; // 3min = 3 * 60 * 1000
// time between states
const int dutyCycleHistoryLength = 100;
unsigned long timeOff[dutyCycleHistoryLength] = {};
unsigned long timeOn[dutyCycleHistoryLength] = {};
unsigned long lastOff = 0;
byte timeOffIndex = 0;
unsigned long lastOn = 0;
byte timeOnIndex = 0;
bool timOnIndexMaxed = false;
//
WiFiUDP ntpUDP;
...
void loop()
{
    ...
    if (historyTracking && (millis() - historyCheckTime) > historyTrackingTimeInterval) { // 3 min
        updateTempHistory();
...
void calcAverageTemp()
{
    float totalTemp = 0;
    if (historyIndex > 0) {
        if (!historyIndexMaxed) {
            for (byte i = 0; i < historyIndex; i++)
                totalTemp += tempHistory[i]; // index is always 1 greater than array data length
            average_temp = totalTemp / float(historyIndex);
        } else {
            for (byte i = 0; i < tempHistoryLength; i++)
                totalTemp += tempHistory[i]; // index is always 1 greater than array data length
            average_temp = totalTemp / float(tempHistoryLength);
        }
    } else
        average_temp = 0;
}
...
void getDutyCycle()
{
    if (timeOnIndex > 0 && timeOffIndex > 0) {
        ...
        if (!timOnIndexMaxed) { // array is not filled with data
            for (int i = 0; i < timeOnIndex; i++)
                timeOnTotal += timeOn[i];
            for (int i = 0; i < timeOnIndex; i++)
                timeOffTotal += timeOff[i];
        } else { // timeOn array is filled with data
            for (int i = 0; i < dutyCycleHistoryLength; i++)
                timeOnTotal += timeOn[i];
            for (int i = 0; i < dutyCycleHistoryLength; i++)
                timeOffTotal += timeOff[i]; // timeonIndex used so that arrays have the same length
        }
        dutyCycle = 100 * (float(timeOnTotal) / float(timeOnTotal + timeOffTotal));
    } else
        dutyCycle = 0;
}
// update history
void updateTempHistory()
{
    ...
    historyIndex++;
    if (historyIndex == (sizeof(tempHistoryTime) / sizeof(tempHistoryTime[0]))) {
        historyIndex = 0;
...
void getHistory()
{
    ...
    if (!historyIndexMaxed) {
        for (int i = 0; i < historyIndex; i++) { // for graph
            if (i == (historyIndex - 1))
                time += "\"" + String(tempHistoryTime[i]) + "\"";
            else
                time += "\"" + String(tempHistoryTime[i]) + "\",";
        }
        for (int i = 0; i < historyIndex; i++) { // for graph
            if (i == (historyIndex - 1))
                temp += String(tempHistory[i]);
            else
                temp += String(tempHistory[i]) + ",";
        }
        for (int i = 0; i < historyIndex; i++)
            res += "{" + String(tempHistoryTime[i]) + "," + String(tempHistory[i]) + "}";
    } else { // data array is full so fetch old data first then latest data
        // rev 9.3
        // time
        for (int i = historyIndex; i < tempHistoryLength; i++) {
            // if (i == (tempHistoryLength - 1)) time += "\"" + String(tempHistoryTime[i]) + "\"";
            time += "\"" + String(tempHistoryTime[i]) + "\",";
        }
        for (int i = 0; i < historyIndex; i++) {
            if (i == (historyIndex - 1))
        ...
        // temp
        for (int i = historyIndex; i < tempHistoryLength; i++) {
            // if (i == (tempHistoryLength - 1))  temp += String(tempHistory[i]);
            temp += String(tempHistory[i]) + ",";
        }
        for (int i = 0; i < historyIndex; i++) {
            if (i == (historyIndex - 1))
    ...
    res += "</p><p>Average Temp: " + String(average_temp) + "</p>"
        "<p>Duty Cycle %: "  + String(dutyCycle) + "</p>"
        //"<p> historyIndex: " + String(historyIndex) + "</p>"
        "<p> timeOnIndex: "  + String(timeOnIndex) + "</p>";
    //
        ...
        if (heatOrAC == 1) { // heat selected
            ...
            if (dhtTemp < lowerLimit) {
                ...
                if (digitalRead(relayPin1) == HIGH & millis() - lastOff > 300000) { // 5 min
                    ...
                    timeOffIndex++;
                    if (timeOffIndex == (sizeof(timeOff) / sizeof(timeOff[0])))
                        timeOffIndex = 0;
                }
                ...
                if (digitalRead(relayPin1) == LOW) {
                    ...
                    timeOnIndex++;
                    if (timeOnIndex == (sizeof(timeOn) / sizeof(timeOn[0]))) {
                        timeOnIndex = 0;
                        timOnIndexMaxed = true;
                    }
                }
            } else { // do nothing
                delay(10);
...
void tempRelaySetup()
{
    // ds18Temp, onTemp, offTemp
    String setuphtml = "<!DOCTYPE HTML><html><head>"
                       "<title>Relay Temp Setup</title>"
                       "<meta name=\"viewport\" content=\"width=device-width, initial-scale=1\">"
                       "</head><body><p>current relay config: </br>HVAC status: "
        + String(heatOrAC) + "</br>Target temp: " + String(targetTemp) + "</br></p></b>"
...
void jsonData()
{
    ...
    String output = "{"
        ...
        + timeClient.getFormattedTime() + "\","
                                          "\"timeTempSchedule\":\""
        + String(timeTempSchedule) + "\","
                                     "\"timeChange0\":\""
        + timeChange0[0] + "," + timeChange0[1] + "\","
                                                  "\"timeChange1\":\""
        + timeChange1[0] + "," + timeChange1[1] + "\","
                                                  "\"historyTracking\":\""
        + String(historyTracking) + "\","
    ...
    output += "\"Wifi_Power_dBm\":\"20\","
              "\"Wifi_PhyMode\":\""
        + String(WiFi.getPhyMode()) + "\"," // 3:
                                      "\"Wifi_Sleep_Mode\":\""
        + String(WiFi.getSleepMode()) + "\"," // 1: light_sleep
                                        "\"Wifi_IP_addr\":\""
...
void argData()
{
    ...
    if (httpServer.arg("timeTempSchedule") != "") {
        ...
        if (value == 1)
            timeTempSchedule = true;
        else
            timeTempSchedule = false;
    }
...
void acAndHeatoff()
{
    if (digitalRead(relayPin1) == LOW) { // heat off
        digitalWrite(relayPin1, HIGH);
        lastOff = millis();
    }
    if (digitalRead(relayPin2) == LOW) { // ac off
        digitalWrite(relayPin2, HIGH);
        lastOff = millis();
    }
}
...
void updateTempLimits()
{
    if (heatOrAC == 1) {
        // heat
        upperLimit = targetTemp + 1.0;
        lowerLimit = targetTemp - 0.5;
    } else if (heatOrAC == 2) {
        // ac
        upperLimit = targetTemp + 0.3;
        lowerLimit = targetTemp - 0.4;
    } else { // not used
        upperLimit = targetTemp + 0.5;
        lowerLimit = targetTemp - 0.5;
    }
}
